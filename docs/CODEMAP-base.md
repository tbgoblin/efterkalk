...
Seneste rapportændring (2026-09-16): Omsætning vises nu altid i ét diagram med én legenda for hele den valgte periode (1-36 måneder); den tidligere 12-månedersopdeling nedenfor er erstattet. Browserkontrol: 1/12/13/15/24/36 måneder med kreditter og alle nettoværdier; ingen labeloverlap ved 15/36 måneder. A4-landskab med 15 måneder: ét panel på 272 px inden for 726 px sidehøjde. Klientversion `ledelsesrapport.js?v=7`.
...
Ledelsesrapport (2026-09-16): `assets/ledelsesrapport.html` og `assets/js/ledelsesrapport.js`; GET `/ledelsesrapport/config` og `/ledelsesrapport/data` kræver `ledelsesrapport`-rettighed. `from/to` er inklusive omsætningsmåneder; uafhængige `orderFrom/orderTo` bruger `YYYY-Www` med ISO-validering og sendes til Ordreindgang som `fraWeek/tilWeek`. Uden ugeparametre bevares den tidligere månedsafledte periode. Ordreindgang viser ordresøjler/-linje, MA3, budget, anomali-markører og periodens `kpis.avgSumOrd`; Tilbud er skjult, kun Ordre får uge-værdilabels. `report-charts.js` deler budgetmål, budgetrækker, ferieuger og MA3 med originalmodulet. Rapporten læser den eksisterende lokale budgetkonfiguration og henter fælles ferieindstillinger ved generering (lokal fallback); ingen indstillinger ændres. Alle valgte uger vises i ét diagram med tilpasset søjlebredde; over 20 uger roteres uge- og værdilabels. Ingen opdeling ved 16 uger. Omsætning opdeles i 12 måneder. Belastning returnerer uændrede `load.rows` i minutter og deler `belastningCluster/loadDays` med originalmodulet (daglig kapacitet, reservationer, aften og rest før i dag). Hver ressource beholder alle dage i ét diagram med tilpasset søjlebredde; over 32 dagskolonner bruges fuld sidebredde. Alle ressourcer medtages, fire halve eller to fuldbrede diagrammer pr. A4-landskabsside. VIA-sektionen viser samlet kendt kost og `totalSales` som forventet salg for de åbne ordrer; resterende salg er separat, ufuldstændigt kostgrundlag markeres. Test: `test/ledelsesrapport.test.js`; browserfixtures kontrollerer SVG-geometri, budget/ferier, MA3, printmål ved 25/32/33/180 dage og ét ugendiagram uden labeloverlap ved 17/20/21/52/157 uger. Backendændringer kræver genstart; Node-suite ikke kørt i editorsessionen.
